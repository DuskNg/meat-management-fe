// meat-management-fe/api/submit-meta.js
// Serverless function trên Vercel để trả về Open Graph meta tags riêng cho trang Gửi ảnh hóa đơn, video công nợ khi chia sẻ qua Zalo/Facebook
const fs = require('fs');
...
module.exports = async (req, res) => {
  try {
    ...
    if (!html) {
      ...
      if (host) {
        try {
          ...
          console.warn('[OG-Submit-Meta] Lỗi fetch loopback index.html:', fetchErr.message);
        }
      }
    }
    ...
    console.error('[OG-Submit-Meta] Lỗi khi xử lý submit metadata:', error);
    return res.redirect(302, '/index.html');
  }
};
